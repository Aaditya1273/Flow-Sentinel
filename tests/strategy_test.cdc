...
access(all) fun setup() {
    ...
    let err2 = Test.deployContract(name: "LiquidStakingStrategy", path: "../contracts/strategies/LiquidStakingStrategy.cdc", arguments: [])
    Test.expect(err2, Test.beNil())

    let err3 = Test.deployContract(name: "YieldFarmingStrategy", path: "../contracts/strategies/YieldFarmingStrategy.cdc", arguments: [])
    Test.expect(err3, Test.beNil())

    let err4 = Test.deployContract(name: "ArbitrageStrategy", path: "../contracts/strategies/ArbitrageStrategy.cdc", arguments: [])
...
// NOTE: executeStrategy with vaultBalance=0 will fail its pre-condition
// "Vault balance must be greater than zero" — this is correct behavior
// enforced by Cadence's pre-condition system and doesn't need a separate test.
